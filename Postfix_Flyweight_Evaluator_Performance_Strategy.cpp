...
#include "Postfix_Flyweight_Evaluator_Performance_Strategy.h"                 // for Expression Evaluator.

#define DEFAULT_SIZE 10                     // A Default size so that the array need not be resized.
// everytime an element is added.

//
// Postfix_Flyweight_Evaluator_Performance_Strategy
//
Postfix_Flyweight_Evaluator_Performance_Strategy::Postfix_Flyweight_Evaluator_Performance_Strategy (void)
	:postfix(0),
	iter(postfix),
	factory(),
	delete_flag(false)
...
Postfix_Flyweight_Evaluator_Performance_Strategy::~Postfix_Flyweight_Evaluator_Performance_Strategy (void)
{
	...
	{
		// checking and deleting only the number commands as the other commands
		// are deleted in the flyweight factory.
		if (postfix[i]->token_type()=="n_")
		{
			delete postfix[i];
		}
	}
...
//
int Postfix_Flyweight_Evaluator_Performance_Strategy::calculator (const std::string & infix)
{
	try
	{
		int result=0;
...
// infix_to_postfix
//
bool Postfix_Flyweight_Evaluator_Performance_Strategy::parse_expr (const std::string & infix)
{
...
// evaluate_postfix
//
bool Postfix_Flyweight_Evaluator_Performance_Strategy::evaluate_expr (int & result)
{
...
// is_number
//
bool Postfix_Flyweight_Evaluator_Performance_Strategy::is_number (const std::string & token, size_t & count, std::string & token_previous, int & value)
{
	...
	if(!(num_temp_1 >> num).fail())
	{
		...
		// if the token is not a valid expression (alphanumeric) then throw exception.
		else
		{
			delete_flag=true;
			return false;
